...
function campus_rooms_init() {
  ...
  // Register libraries
  $base_dir = elgg_get_plugins_path() . 'rev_cart_store/lib';
  ...
  elgg_register_page_handler('cart', 'rev_cart_store_page_handler');

  elgg_register_action("gallery/add/item", __DIR__ . "/actions/gallery/add/item.php");
...
function rev_cart_store_page_handler($segments) {

  if (!isset($segments[0])) {
    $segments = array('gallery');
  }
  
  switch ($segments[0]) {
    case 'gallery':
    $gallery_page = $segments[1] ;
    if($gallery_page == 'add' && $segments[2] == 'item')
    {
      echo elgg_view_resource('cart/gallery/add/item');
    }
    break;
    ...
    echo elgg_view_resource('cart/room', $resource_vars);
    break;
