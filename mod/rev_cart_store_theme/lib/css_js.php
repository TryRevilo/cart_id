...
function rev_theme_init_css() {
	...
	$css_overrides = 'mod/rev_cart_store_theme/vendors/css/rev_css_overrides.css';
	elgg_register_css('css_overrides', $css_overrides);
	elgg_load_css('css_overrides');

	$normalize = 'mod/rev_cart_store_theme/vendors/normalize.css';
	elgg_register_css('normalize', $normalize);
	elgg_load_css('normalize');

	$css_base = 'mod/rev_cart_store_theme/vendors/twitter_bootstrap/css';

	$bootstrap = $css_base . '/bootstrap.min.css';
	elgg_register_css('bootstrap', $bootstrap);
	elgg_load_css('bootstrap');

	$bootstrap_theme =$css_base . '/bootstrap-theme.min.css';
	elgg_register_css('bootstrap-theme', $bootstrap_theme);
	elgg_load_css('bootstrap-theme');

  	// JS

	$file_style_base = 'mod/rev_cart_store_theme/vendors/bootstrap-filestyle-1.2.1/src/bootstrap-filestyle.min.js';
	elgg_register_js('file_style', $file_style_base);
	elgg_load_js('file_style');

	$autosize_master = 'mod/rev_cart_store_theme/vendors/autosize-master/dist/autosize.js';
	elgg_register_js('autosize_master', $autosize_master);
	elgg_load_js('autosize_master');

	// Bootstrap

	$bootstrap_js_base = 'mod/rev_cart_store_theme/vendors/twitter_bootstrap/js';
	...
	$textcomplete = 'mod/rev_cart_store_theme/vendors/JQuery/jquery-textcomplete-master/dist/jquery.textcomplete.min.js';
	elgg_register_js('textcomplete', $textcomplete);
	elgg_load_js('textcomplete');

	// js Auto resize Text Area

	$auto_resize_text_area = 'mod/rev_cart_store_theme/vendors/JQuery/auto_resize_text_area.js';
